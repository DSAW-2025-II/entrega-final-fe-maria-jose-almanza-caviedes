...
// - Surfaces misconfiguration when OPENROUTESERVICE_KEY is missing (→ 500 with params).
...
// Mock axios to avoid performing real HTTP requests to OpenRouteService.
...
const SAMPLE_DIRECTIONS_RESPONSE = {
  features: [
    {
      properties: {
        summary: {
...
describe("Maps routes", () => {
  ...
  // Ensures proper error handling when OPENROUTESERVICE_KEY is not set.
  ...
  it("should 500 without OPENROUTESERVICE_KEY when params are present", async () => {
    const prev = process.env.OPENROUTESERVICE_KEY; // Backup current key (if any)
    delete process.env.OPENROUTESERVICE_KEY;       // Simulate missing configuration
    ...
    if (prev) process.env.OPENROUTESERVICE_KEY = prev;
  });

  it("returns parsed payload in /maps/calculate", async () => {
    process.env.OPENROUTESERVICE_KEY = "test-key";
    axiosGet.mockResolvedValue({ data: SAMPLE_DIRECTIONS_RESPONSE });
  ...
  it("uses cache when available", async () => {
    process.env.OPENROUTESERVICE_KEY = "test-key";
    mockedRedis.get.mockResolvedValueOnce(JSON.stringify(SAMPLE_DIRECTIONS_RESPONSE));
  ...
  it("maps provider rate limit to 429", async () => {
    ...
    rateLimitError.response = { status: 429, data: { error: { message: "Too many requests" } } };
    ...
    expect(res.body.providerStatus).toBe("HTTP_429");
  });
